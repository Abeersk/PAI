...
**Control Barrier Functions (CBFs)**: A function h(x) is a control barrier function if there exists an extended class K function α such that:

inf_u [L_f h(x) + L_g h(x) u + α(h(x))] ≥ 0

This ensures forward invariance of the safe set {x | h(x) ≥ 0}.

**Safety Filters**: Real-time safety filters modify control commands to ensure constraint satisfaction:

u_safe = argmin_u ||u - u_nominal||²
